...
public class TextDrawable extends Drawable {
    ...
    public TextDrawable(Context ctx, String text, ColorStateList color, float textSize, VerticalAlignment verticalAlignment) {
        ...
        this.textPaint.setTypeface(Typeface.defaultFromStyle(Typeface.BOLD));
        measureSize();
        setBounds(20, 0, iWidth, iHeight);
        this.color = color;
    ...
    protected void measureSize() {
        ...
        iWidth = (int) (textPaint.measureText(text));
        iHeight = (int) (textPaint.descent() + ascent);
    ...
    public void draw(Canvas canvas) {
        ...
        if (text != null && !text.isEmpty()) {
            ...
            final float x = bounds.width() >= iWidth ? bounds.centerX() : iWidth * 0.7f;
            float y = 0;
            switch (verticalAlignment) {
                case BASELINE:
                    y = (bounds.height() - iHeight) * 0.7f + ascent;
                    break;
                case TOP:
                    y = bounds.height();
                    break;
